...
// returns:
// - 0, if this sand cannot be dropped
// - 1, if the sand ends in a stable position
// - 2, if the sand can fall freely
int drop_sand(int start_x, int start_y)
{
    ...
    if (grid.find(start_x) != grid.end())
    {
        ...
        else
        {
            ...
            int diag_left = drop_sand(start_x - 1, next_y);
            if (diag_left != 0)
            {
                return diag_left;
            }
            else
            {
                ...
                int diag_right = drop_sand(start_x + 1, next_y);
                if (diag_right != 0)
                {
                    return diag_right;
                }
    ...
    // no obstructions in the same column, sand can fall freely
    else
    {
        return 2;
    }
